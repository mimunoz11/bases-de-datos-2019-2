...
planificar (_presupuesto integer, _comuna varchar, _nombre_ong varchar)
...
BEGIN
    ...
    IF array_length(pid_planificados, 1) > 0 THEN
        presupuesto_individual := ROUND(_presupuesto / array_length(pid_planificados, 1));
    END IF;
        ...
        FOR proyecto_planificado IN (SELECT * FROM Proyectos WHERE pid = pid_planificado) LOOP
            ...
            IF opcion_tipo = 'marcha' THEN 
                ...
                FOR id_movilizacion_creada IN (
                    SELECT * FROM dblink('dbname=grupo6 host=localhost port=5432 user=grupo6 password=grupo6','INSERT INTO movilizacion VALUES(default, ' || presupuesto_individual || ', ''' || opcion_tipo || ''', '''  || fecha_guardar || ''', ''' || _nombre_ong || ''', ''' || proyecto_planificado.nombre || ''') returning id;') AS Ides (id integer)
                    ) LOOP
                    PERFORM dblink('dbname=grupo6 host=localhost port=5432 user=grupo6 password=grupo6','INSERT INTO marcha VALUES (' || id_movilizacion_creada.id || ', ' || n_esperado_guardar || ', ''' || lugar_guardar || ''');');
            ...
            ELSIF opcion_tipo = 'redes sociales' THEN
                ...
                FOR id_movilizacion_creada IN (
                    ...
                    PERFORM dblink('dbname=grupo6 host=localhost port=5432 user=grupo6 password=grupo6','INSERT INTO movredsocial VALUES (' || id_movilizacion_creada.id || ', ''' || contenido_guardar || ''', ' || duracion_guardar || ');');
...
RETURN QUERY EXECUTE '(SELECT * FROM dblink($1, $2)
            ...
            user=grupo6 password=grupo6', 'SELECT movilizacion.id, ong, proyecto, presupuesto, tipo, fecha, estimacion, lugar, tipo_contenido, duracion from movilizacion full outer join marcha on movilizacion.id = marcha.id full outer join movredsocial on movilizacion.id = movredsocial.id
            WHERE movilizacion.id = ANY(array[' || array_to_string(id_movilizaciones_creadas, ',') || ']::integer[])';
            RETURN;
END;
$$ language plpgsql;