        ...
        FOR proyecto_planificado IN (SELECT * FROM Proyectos WHERE pid = pid_planificado) LOOP
            ...
            IF opcion_tipo = 'marcha' THEN 
                ...
                FOR id_movilizacion_creada IN (
                    SELECT * FROM dblink('dbname=grupo6 host=localhost port=5432 user=grupo6 password=grupo6','INSERT INTO movilizacion VALUES(default, ' || presupuesto_individual || ', ''' || opcion_tipo || ''', '''  || fecha_guardar || ''', ''' || _nombre_ong || ''', ''' || nombre_proyecto || ''') returning id;') AS Ides (id integer)
                    ) LOOP
